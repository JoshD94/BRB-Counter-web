...
function calculate() {
    ...
    semester_completion.innerHTML = "Semester completed:\t" + round(percentage_semester * 100, 2) + "%"
    brb_used.innerHTML = "BRBs used:\t" + round(percentage_brbs * 100, 2) + "%"
    weekly_usage.innerHTML = "Projected weekly usage:\t$" + (current_amount / days_left * 7).toFixed(2)
    daily_usage.innerHTML = "Projected daily usage:\t$" + (current_amount / days_left).toFixed(2)
    if (percentage_brbs < percentage_semester) {
        comment.innerHTML = "You are under-spending your BRBs!! As it is Spring semester, you might want to spend more because your leftover BRBs will not be transfered over to next semester!"
    } else if (percentage_brbs > percentage_semester) {
        comment.innerHTML = "You are over-spending your BRBs!! Maybe try hold back on spendings? :D"
    } else {
        comment.innerHTML = "Woah, you're spending the exact amount to keep up with the semester!! Keep it up!"
    }
